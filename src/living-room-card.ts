...
export interface LivingRoomCardConfig extends LovelaceCardConfig {
  main_entity: string;
...
export class LivingRoomCard extends LitElement implements LovelaceCard {
  ...
  private _renderSwitch(entry: SwitchEntry) {
    ...
    return html`
      <div class="${cls}" @click=${() => this._toggle(entry.entity)} @contextmenu=${(ev: Event) => { ev.preventDefault(); this._moreInfo(entry.entity); }}>
  ...
  protected render() {
    ...
    return html`
      <ha-card class="lr-wrapper">
        ...
        ${rows.map(
          (row) => html`<div class="switch-row">${row.map((s) => this._renderSwitch(s))}</div>`
        )}
      </ha-card>
    `;
  }
